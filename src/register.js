            ...
            <Row className="mb-3">
              <Form.Group as={Col} md="4" controlId="validationFormik02" className="position-relative">
                ...
                <Form.Select name="facility" value={values.facility} isValid={touched.facility && !errors.facility} isInvalid={touched.facility && errors.facility} onChange={(e) => { values.facility = e.target.value; props.handleFacility(values.facility)}} onBlur={handleBlur}>
                  ...
                  { _.map(props.facilityNames, (f) => { return (<option key={f} value={f}>{f}</option>)}) }
                </Form.Select>
  ...
  render() {
    return (
      <Modal show={this.props.show} onHide={this.handleClose} size="lg">
        ...
        <UserIdForm handleSubmit={this.requestAccount} preferredUserName={_.split(this.props.eppn, "@")[0]} handleClose={this.handleClose} eppn={this.props.eppn} facilityNames={this.props.facilityNames} handleFacility={this.props.handleFacility} selectedFacility={this.props.selectedFacility}/>
      </Modal>
      )
  }
}
...
export default function RegisterUser(props) {
  ...
  const requestAccount = (preferredUserName) => {
    console.log("Account requested for eppn " + eppn + " and preferred userid " + preferredUserName);
    requestUserAccount({ variables: { request: { reqtype: "UserAccount", eppn: eppn, preferredUserName: preferredUserName, "facilityname": facility }}, refetchQueries: [ FACNAMES, 'amIRegistered' ]});
    setShow(false);
  };
  ...
  return (
    ...
    <div className="registeruser d-flex flex-column">
      ...
      <ReqUserAccount show={show} setShow={setShow} eppn={eppn} requestUserAccount={requestAccount} facilityNames={data.facilityNames} handleFacility={handleFacility} selectedFacility={facility} />
      <Footer/>
    </div>
    </>
  );
